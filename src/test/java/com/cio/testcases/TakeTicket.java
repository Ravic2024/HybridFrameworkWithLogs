...
public class TakeTicket extends TestBase {
	@Test(dataProviderClass = TestUtils.class, dataProvider = "dp")
	public void PSDeptTake(String Subject, String department, String Application, String Description) throws Throwable {
		...
		try {
			...
			test.log(Status.PASS, "Application Dropdown Displayed Successfully");
			waitForElement("PSAPPDDown_XPATH");
			Assert.assertTrue(isElementPresent(By.xpath(OR.getProperty("PSAPPDDown_XPATH"))));
			driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(5));
			HighlightWebElement("PSAPPDDown_XPATH");
			WebElement tic = driver.findElement(By.xpath(OR.getProperty("PSAPPDDown_XPATH")));
			Actions action = new Actions(driver);
			action.moveToElement(tic).perform();
			Click("PSAPPDDown_XPATH");
